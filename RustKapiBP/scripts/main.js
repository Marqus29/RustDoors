...
const SECURITY_TERMINAL_ID = "rust:security_terminal";
const DEFAULT_DOOR_TIMER = 3;
const REPAIR_ITEM_ID = "minecraft:iron_ingot";
const REPAIR_AMOUNT = 150;
...
// -------------------------------------------------------------
// CACHE VE YARDIMCI FONKSİYONLAR
// -------------------------------------------------------------
...
function getDoorType(typeId) {
    if (!typeId || !typeId.startsWith("rust:bunker_door")) return "bunker";
    const clean = typeId.replace("_top", "").replace("rust:bunker_door_", "").replace("rust:bunker_door", "");
    return clean.length > 0 ? clean : "bunker";
}

function getMaxHpForDoor(typeId) {
    const type = getDoorType(typeId);
    return DOOR_MAX_HP[type] || 800;
}
...
function removeLockCompletely(lowerBlock, primaryKey) {
    saveDoorData(primaryKey, null, false);

    const adjacent = findAdjacentDoor(lowerBlock);
    if (adjacent && adjacent.lowerBlock) {
        const adjKey = getDoorKey(adjacent.lowerBlock);
        saveDoorData(adjKey, null, false);
    }
}
...
// -------------------------------------------------------------
// TAMİR (DEMİR KÜLÇESİYLE EĞİLEREK SAĞ TIK)
// -------------------------------------------------------------
function handleDoorRepair(player, lowerBlock, doorKey, doorData) {
    const maxHp = getMaxHpForDoor(lowerBlock.typeId);
    if (!doorData.hp) doorData.hp = maxHp;

    if (doorData.hp >= maxHp) {
        player.playSound("note.bass", { pitch: 1.5, volume: 0.8 });
        player.onScreenDisplay.setActionBar(`§a[TAMİR] §7Kapı zaten tam cana sahip! (§a${maxHp}/${maxHp} HP§7)`);
        releasePlayer(player);
        return;
    }

    const container = player.getComponent("minecraft:inventory")?.container;
    if (!container) { releasePlayer(player); return; }

    const heldItem = container.getItem(player.selectedSlotIndex);
    if (!heldItem || heldItem.typeId !== REPAIR_ITEM_ID) { releasePlayer(player); return; }
    ...
    doorData.hp = Math.min(maxHp, doorData.hp + REPAIR_AMOUNT);
    saveDoorData(doorKey, doorData, true);

    try { lowerBlock.dimension.playSound("rust.door_repair", lowerBlock.location, { pitch: 1.0, volume: 1.0 }); } 
    catch (e) { lowerBlock.dimension.playSound("random.anvil_use", lowerBlock.location, { pitch: 1.4, volume: 0.8 }); }

    try {
        lowerBlock.dimension.spawnParticle("minecraft:crit_particle", {
            x: lowerBlock.x + 0.5, y: lowerBlock.y + 1.0, z: lowerBlock.z + 0.5
        });
    } catch (e) {}

    const percent = Math.round((doorData.hp / maxHp) * 100);
    const filledBlocks = Math.round((doorData.hp / maxHp) * 10);
    const emptyBlocks = 10 - filledBlocks;
    const bar = "§a" + "■".repeat(filledBlocks) + "§7" + "□".repeat(emptyBlocks);

    player.onScreenDisplay.setActionBar(`§e[TAMİR] ${bar} §f%${percent} §8(${doorData.hp}/${maxHp} HP)`);
    releasePlayer(player);
}

// -------------------------------------------------------------
// PATLAMA / RAID MEKANİĞİ
// -------------------------------------------------------------
world.beforeEvents.explosion.subscribe((event) => {
    const dim = event.dimension;
    const blocks = event.getImpactedBlocks() || [];
    
    const expLoc = event.source ? event.source.location : (blocks.length > 0 ? blocks[0].location : null);
    if (!expLoc) return;

    // 1. KAPI HASAR HESAPLAMASI
    const damagedDoors = [];
    const blastRadius = 6.0;

    for (const [key, cache] of doorCache.entries()) {
        if (cache.dim !== dim.id || !cache.data || !cache.data.pin) continue;

        const doorX = cache.x + 0.5;
        const doorY = cache.y + 1.0;
        const doorZ = cache.z + 0.5;

        const distSq = (expLoc.x - doorX) ** 2 + (expLoc.y - doorY) ** 2 + (expLoc.z - doorZ) ** 2;
        if (distSq <= blastRadius * blastRadius) {
            damagedDoors.push({ key, cache, dist: Math.sqrt(distSq) });
        }
    }

    for (const item of damagedDoors) {
        const { key, cache, dist } = item;
        const doorData = cache.data;
        
        let doorBlock = null;
        try { doorBlock = dim.getBlock({ x: cache.x, y: cache.y, z: cache.z }); } catch (e) {}
        const maxHp = getMaxHpForDoor(doorBlock?.typeId || "rust:bunker_door");
        
        if (doorData.hp === undefined) doorData.hp = maxHp;

        const damage = Math.max(100, Math.round((1 - (dist / blastRadius)) * 350));
        doorData.hp -= damage;
        const doorDisplayName = doorData.name || `Kapı [${cache.x}, ${cache.y}, ${cache.z}]`;

        if (doorData.hp <= 0) {
            doorData.hp = 0;
            // Bellekte anında sıfırla ki alt satırdaki alan koruması kapının öldüğünü anlasın
            cache.data = null; 

            // world save işlemlerini ertelemek ZORUNDAYIZ (ReadOnly Error fix)
            system.run(() => {
                saveDoorData(key, null, true);
                try {
                    if (doorBlock) {
                        const { lowerBlock, upperBlock } = getDoorBlocks(doorBlock);
                        if (lowerBlock) lowerBlock.setType("minecraft:air");
                        if (upperBlock) upperBlock.setType("minecraft:air");

                        const adj = findAdjacentDoor(lowerBlock || doorBlock);
                        if (adj) {
                            if (adj.lowerBlock) adj.lowerBlock.setType("minecraft:air");
                            if (adj.upperBlock) adj.upperBlock.setType("minecraft:air");
                        }
                    }
                } catch (e) {}
                notifyOwner(doorData.owner, `§4§l[RAID UYARISI] §e${doorDisplayName}§c patlayıcıyla tamamen patlatıldı!`, "§4§lKAPI PATLATILDI!", `§e${doorDisplayName} §cyıkıldı!`);
            });
        } else {
            // Bellekte güncellendi, sadece world save'i erteliyoruz
            system.run(() => {
                saveDoorData(key, doorData, true);
                notifyOwner(doorData.owner, `§e[RAID HASARI] §e${doorDisplayName}§c hasar aldı! Kalan Can: §e${doorData.hp}/${maxHp} HP`, "§eKAPI HASAR ALDI!", `§e${doorDisplayName}: §c${doorData.hp} HP`);
            });
        }
    }

    // 2. KORUNAN ALANLARI BELİRLE
    const protectedDoors = [];
    for (const [key, cache] of doorCache.entries()) {
        if (cache.dim === dim.id && cache.data && (cache.data.hp === undefined || cache.data.hp > 0)) {
            protectedDoors.push({
                x: cache.x,
                y: cache.y,
                z: cache.z,
    ...
    // 3. FİZİKSEL PATLAMA FİLTRESİ
    const safeBlocks = [];
    for (const b of blocks) {
        if (isAnyBunkerDoor(b.typeId)) continue;

        try {
            const above = dim.getBlock({ x: b.x, y: b.y + 1, z: b.z });
            if (above && isAnyBunkerDoor(above.typeId)) continue;
        } catch (e) {}
        ...
        if (!inProtectedZone) safeBlocks.push(b);
    }
...
function openAllDoorsAndCardsMenu(player) {
    ...
    const form = new ActionFormData().title("§8[GÜVENLİK TERMİNALİ]§r");
...
function openDoorManagementMenu(player, doorObj) {
    ...
    new ActionFormData()
        ...
        .show(player).then((res) => {
            ...
            if (res.selection === 0) {
                system.run(() => promptEncodeCardOnTerminal(player, doorObj));
...
function promptEncodeCardOnTerminal(player, doorObj) {
...
world.beforeEvents.itemUseOn.subscribe((event) => {
    ...
    if (itemStack && (itemStack.typeId === "minecraft:flint_and_steel" || itemStack.typeId === "minecraft:fire_charge")) {
        const protectedDoor = findNearbyLockedDoor(block.dimension, block.location, 5);
...
// -------------------------------------------------------------
// BLOK ETKİLEŞİMİ (KAPI, TAMİR, TERMİNAL)
// -------------------------------------------------------------
world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
    const { block, player } = event;

    if (block.typeId === SECURITY_TERMINAL_ID) {
        event.cancel = true;
    ...
    if (block.typeId.startsWith("rust:bunker_door")) {
        ...
        if (player.isSneaking && heldItem && heldItem.typeId === REPAIR_ITEM_ID) {
            const { lowerBlock } = getDoorBlocks(block);
            if (lowerBlock) {
                const { key, data } = getDoorData(lowerBlock);
                if (data && data.pin && isPlayerAuthorized(data, player.id)) {
                    system.run(() => { handleDoorRepair(player, lowerBlock, key, data); });
                    return;
                }
            }
        }
...
world.beforeEvents.playerBreakBlock.subscribe((event) => {
    ...
    if (block.typeId.startsWith("rust:bunker_door")) {
        ...
        if (data && data.pin) {
            ...
            system.run(() => {
                player.playSound("note.bass", { pitch: 0.5, volume: 1.0 });
                player.onScreenDisplay.setActionBar("§c[!] Kapı kilitli! Sökmek için eğilip menüden kilidi sökün.");
                if (data.owner && data.owner !== player.id) {
                    notifyOwner(data.owner, `§4§l[BASKIN ALARMI] §c${player.nameTag} kapını kırmaya çalışıyor!\n§eKonum: §fX:${lowerBlock.x}, Y:${lowerBlock.y}, Z:${lowerBlock.z}`, "§4§lKAPI ZORLANIYOR!", `§c${player.nameTag} kapına vuruyor!`);
                }
            });
            return;
        }
...
world.afterEvents.playerBreakBlock.subscribe((event) => {
    ...
    if (isBaseDoor(brokenId)) {
        ...
        const key = getDoorKey(block);
        removeLockCompletely(block, key);
    } else if (isTopDoor(brokenId)) {
        const bottomBlock = block.below();
        const expectedBaseId = brokenId.replace("_top", "");
        if (bottomBlock && bottomBlock.typeId === expectedBaseId) {
            bottomBlock.setType("minecraft:air");
            const key = getDoorKey(bottomBlock);
            removeLockCompletely(bottomBlock, key);
        }
    }
});
    ...
    new ModalFormData().title("§2[KEYPAD KURULUMU]§r")
        ...
        .show(player).then((res) => {
            ...
            if (cleanPin.length > 0) {
                const maxHp = getMaxHpForDoor(lowerBlock.typeId);
                const newDoorData = {
                    pin: cleanPin, name: doorName, owner: player.id, hp: maxHp, timer: DEFAULT_DOOR_TIMER,
...
function openOwnerMenu(player, doorKey, doorData, lowerBlock) {
    ...
    if (isOwner) form.button("Kilidi Sök (Şifreyi Sıfırla)", "textures/items/iron_pickaxe");
    ...
    form.show(player).then((res) => {
        ...
        if (sel === 0) system.run(() => openRenameDoorModal(player, doorKey, doorData, lowerBlock));
            ...
            removeLockCompletely(lowerBlock, doorKey);
            player.playSound("random.break", { pitch: 1.0, volume: 1.0 });
            player.onScreenDisplay.setActionBar("§e[!] Kilit kapıdan tamamen söküldü. Artık kırılabilir!");
            releasePlayer(player);
        } else releasePlayer(player);
    }).catch(() => releasePlayer(player));
}
...
// -------------------------------------------------------------
// TÜRE ÖZEL .OGG SESLERİYLE KAPI TETİKLEME
// -------------------------------------------------------------
function toggleDoor(lowerBlock, upperBlock, doorData) {
    ...
    const doorType = getDoorType(lowerBlock.typeId);
    const soundOpen = `rust.door_open_${doorType}`;
    const soundClose = `rust.door_close_${doorType}`;

    try { lowerBlock.dimension.playSound(soundOpen, lowerBlock.location, { pitch: 1.0, volume: 1.0 }); } catch (e) {}
        ...
        for (const d of doors) {
            try {
                if (d.lower.permutation.getState("rust:open")) {
                    ...
                    if (!soundPlayed) {
                        try { d.lower.dimension.playSound(soundClose, d.lower.location, { pitch: 1.0, volume: 1.0 }); } catch (e) {}
                        soundPlayed = true;
                    }
                }
            } catch (e) {}
...
function openHelpGuide(player) {
    ...
    new ActionFormData().title("§6[RUST DOOR REHBERİ]§r")
        ...
        .button("[SABOTAJ] Alan Koruması & Raid", "textures/items/diamond_chestplate").button("Kapat", "textures/ui/cancel")
        .show(player).then((res) => {
            ...
            else if (res.selection === 1) infoForm.title("§2[RFID KARTLAR]§r").body("§e- Üretim:§7 Tezgâhta boş kart yapın.\n§e- Kodlama:§7 Güvenlik Terminaline sağ tıklayıp kart basın.");
            ...
            else if (res.selection === 4) infoForm.title("§2[RAID & TAMİR]§r").body("§e- Raid:§7 Patlamalar kapıya hasar verir. Can bitince kapı patlar!\n§e- Tamir:§7 Eğilip Demir Külçesiyle vurarak +150 HP tamir edin.");
